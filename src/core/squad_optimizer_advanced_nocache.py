...
Modified version that accepts external client to bypass caching
"""
import pulp
...
class AdvancedSquadOptimizerNoCache:
    """Enhanced squad optimizer using multiple data sources - no cache version"""
    
    def __init__(self):
        self.weights = {
            'base': 0.50,      # Historical/last season 
            'form': 0.0,      # Recent form
            'fixtures': 0.25,  # Fixture difficulty
    ...
    async def optimize_initial_squad_advanced(
        ...
        Args:
            budget: Budget to spend on squad
            client: Optional FPL client to use (for cache control)
        """
        app_logger.info(f"Advanced squad optimization with budget £{budget}m")
        
        # Use provided client or create new one
        if client:
            app_logger.info("Using provided FPL client (cache controlled externally)")
            # Don't use context manager since client is managed externally
            ...
            async with FPLClient() as new_client:
                # Fetch all necessary data
                app_logger.info("Fetching player and fixture data...")
                
                # Get current player data
                bootstrap_data = await new_client.get_bootstrap_data()
                all_players_data = bootstrap_data.get('elements', [])
                teams_data = bootstrap_data.get('teams', [])
                
                # Get fixtures for difficulty analysis
                fixtures_data = await new_client.get_fixtures()
    ...
    def _calculate_base_score(self, player: Player, data: Dict) -> float:
        ...
        if last_season_total > 0:
            ...
            base = (last_season_total * 0.8 + current_points * 0.2) / 38
            
            # Bonus for consistently good players
            if last_season_total > 150:
                base *= 1.2  # 20% bonus for 150+ point players
        else:
    ...
    def _calculate_ownership_score(self, player: Player) -> float:
        ...
        if ownership < 5 and player.form > 5:
            ...
            return 20  # Reduced from 40
        elif ownership > 40:
            return 10  # Template player - reduced from 20
        else:
